An excellent request! Building `relaycode` sounds like a fantastic way to streamline developer workflows with LLMs. I will create the project structure and code based on your detailed requirements, adhering to best practices like type safety, functional patterns, and immutability where appropriate.

Here is the complete codebase for the `relaycode` project.

### `package.json`

This file sets up the project, its dependencies, and the `relay` command-line executable.
...
A strict TypeScript configuration to ensure code quality and type safety.
...
The main entry point for the command-line interface, using `commander` to handle commands and arguments.
...
This file defines all the core data structures and Zod schemas, ensuring end-to-end type safety from configuration and clipboard input through the entire application logic.
...
A central place for all constant string values to avoid "magic strings" in the code.
...
A simple logging utility using `chalk` for colored output to improve readability in the terminal.
...
A utility to handle interactive user confirmation prompts in the terminal.

```typescript // src/utils/prompt.ts
...
A wrapper around Bun's `spawn` process for executing external shell commands like linters and custom scripts.
...
This module handles loading, creating, and validating the `relaycode.config.json` file.
...
The core parsing engine that transforms raw clipboard text from an LLM into structured, actionable operations.
...
This module abstracts all direct file system interactions, such as reading, writing, deleting, snapshotting, and restoring files.
...
Handles all interactions with the `.relaycode` state directory, managing pending and committed transaction files.
...
A simple and robust clipboard watcher that polls for new content and triggers a callback.
...
This is the orchestrator for the entire patch-and-apply workflow, ensuring each change is handled atomically.
...
Contains all the logic for the `relay init` command.
...
Contains the logic for the `relay watch` command, which starts the clipboard monitoring loop.
