...
// A simple set-based diff to calculate line changes.
const calculateLineChanges = (oldContent: string | null, newContent: string) => {
...
const createTransaction = (deps: TransactionDependencies) => {
  ...
  const _rollbackTransaction = async (snapshot: FileSnapshot, reason: string): Promise<void> => {
  ...
  const _applyChangesAndGetStats = async (snapshot: FileSnapshot): Promise<Array<{ added: number, removed: number }> | null> => {
    try {
        ...
        const statPromises = operations.map(async op => {
            ...
            const newContent = op.type === 'write' ? (await readFileContent(op.path, cwd) ?? '') : '';
        ...
        logger.error(`Failed to apply file operations: ${error instanceof Error ? error.message : String(error)}.`);
        return null;
    }
  };

  const _getApprovalStatus = async (): Promise<boolean> => {
  ...
  const execute = async (snapshot: FileSnapshot, startTime: number): Promise<void> => {
    ...
    const opStats = await _applyChangesAndGetStats(snapshot);
    if (!opStats) {
        await _rollbackTransaction(snapshot, 'Apply error');
        return;
    }

    const isApproved = await _getApprovalStatus();
    ...
    if (isApproved) {
        ...
        await _rollbackTransaction(snapshot, 'Changes were not approved or a post-command failed.');
    }
  };
